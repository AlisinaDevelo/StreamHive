...
- **CLI**: `-replicate`, `-put-key`, `-put-data`, `-exit-after-put`, `-peers`, `-peer-reconnect`, `-peer-reconnect-min`, `-peer-reconnect-max`, and `-max-blob-bytes` for static in-memory replication demos and long-lived static-peer nodes.
